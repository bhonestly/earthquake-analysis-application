...
function appendLocationData(features) {
  ...
  features.forEach((feature) => {
    // for each item in the API response array we are grabbing the location.
    let placesArray = feature.properties.place.split("of");
    if (!uniqueLocations.includes(placesArray[1]))
      uniqueLocations.push(placesArray[1]); // for each item in the API response array we are checking if the location is included in the uniqueLocations array.  If it is not in the uniqueLocations array we are pushing the location into that array.
  });
  uniqueLocations.forEach((location) => {
    // for each location in the uniqueLocations array we are creating an option tag, setting the value attribute of the option tag to the location value and appending the option tag to the DOM
